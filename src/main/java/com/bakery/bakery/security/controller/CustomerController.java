...
public class CustomerController {
    ...
    private ResponseEntity<CustomerResponse> associateCustomerSubscription(
            @Valid @RequestBody CustomerSubscriptionRequest request) throws Exception {
        ...
        var subscription = subscriptionService.getById(request.getSubscriptionId())
                .orElseThrow(() -> new ResourceNotFoundExceptionRequest("Subscription not found"));
    ...
    @GetMapping("/{id}")
    private ResponseEntity<CustomerResponse> getById(@PathVariable(name = "id") Long id) throws Exception {
        var entity = customerService.getById(id)
                .orElseThrow(() -> new ResourceNotFoundExceptionRequest("Customer by id not found"));
